        ...
        uint8 MemoryRangeInfoIndex)
{
    ...
    if (pDcmDspWriteMemoryRangeInfo->DcmDspWriteMemorySecurityLevelRefNum != 0u)
    {
        for (index = 0; (index < pDcmDspWriteMemoryRangeInfo->
                DcmDspWriteMemorySecurityLevelRefNum); index++)
        {
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Std_ReturnType, DCM_CODE)Dcm_0x3DMemoryWriteSessionCheck(
        uint8 MemoryIdInfoIndex,
        uint8 MemoryRangeInfoIndex)
{
    uint8 index;
    ...
    {
        for (index = 0; (index < pDcmDspWriteMemoryRangeInfo->
                DcmDspWriteMemoryRangeSessionLevelRefNum); index++)
        {
            if (Dcm_MkCtrl.Dcm_ActiveSes == pDcmDspWriteMemoryRangeInfo->
                    pDcmDspWriteMemoryRangeSessionLevelRow[index])
            {
                ret = E_OK;
            }
        }
...
#endif
    /*min-length check*/
    if ((E_OK == ret) && (DCM_UDS0X3D_REQ_DATA_MINLENGTH >
        ...
        P2VAR(uint8, AUTOMATIC, AUTOMATIC)MemoryAddressMatchNum)
{
    ...
    Dcm_DspCfg.pDcmDspMemory->DcmDspMemoryIdInfoNum; IdInfoIndex++)
    {
        ...
        if (pDcmDspMemoryIdInfo != NULL_PTR)
        {
            ...
            pDcmDspMemoryIdInfo->DcmDspWriteMemoryRangeInfoNum; RangeInfoIndex++)
            {
                pDcmDspWriteMemoryRangeInfo =
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    if (E_OK == ret)
    {
        ...
        ret = DsdInternal_0x3DWriteMemoryRangeCheck(&Dcm_0x3DType,
    ...
    if (E_OK == ret)
    {
        ...
        ret = Dcm_WriteMemory(OpStatus,MemoryIdentifier,MemoryAddress,
                Memorysize,DCM_MemoryData,ErrorCode);
    ...
    if ((E_OK == ret) && (((uint16)((uint16)2 + MemoryAddressSize + MemoryLengthSize))
    ...
    if (E_OK == ret)
    {
        ...
        {
            Dcm_Channel[Offset + 2u +(uint16)index] = Dcm_MsgCtrl[MsgCtrlId].
                    MsgContext.pReqData[2u + index];
        }
