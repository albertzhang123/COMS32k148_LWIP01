        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
...
#endif
    /*min-length check*/
    if ((E_OK == ret) && (DCM_UDS0X38_REQ_DATA_MINLENGTH >
        ...
        if ((modeOfOperation != 0x02u) && (modeOfOperation != 0x05u))
        {
            *Dcm_0x38Type->dataFormatIdentifier = Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[ReqOffset];
            ...
            if (modeOfOperation != 0x04u)
            {
                ...
                {
                    *Dcm_0x38Type->fileSizeParameterLength = (uint16)Dcm_MsgCtrl[MsgCtrlId].
                            MsgContext.pReqData[ReqOffset];
                }
                ReqOffset += 1u;
                Dcm_0x38Type->fileSizeUncompressedOrDirInfoLength = &(Dcm_MsgCtrl[MsgCtrlId].
                        MsgContext.pReqData[ReqOffset]);
    ...
    if (E_OK == ret)
    {
        ret = Dcm_ProcessRequestFileTransfer(OpStatus,modeOfOperation,
                *Dcm_0x38Type->fileSizeParameterLength,filePathAndName,*Dcm_0x38Type->dataFormatIdentifier,
                Dcm_0x38Type->fileSizeUncompressedOrDirInfoLength,
                Dcm_0x38Type->fileSizeCompressed,Dcm_0x38Type->BlockLength,ErrorCode);
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(void, DCM_CODE)Dcm_UDS0x38_Tx(
        uint8  ProtocolCtrlId,
        uint16 ResOffset,
        uint32 BlockLength)
{
