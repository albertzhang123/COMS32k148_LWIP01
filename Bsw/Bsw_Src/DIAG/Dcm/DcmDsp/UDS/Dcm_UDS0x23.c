        ...
        uint8 MemoryRangeInfoIndex)
{
    ...
    if (pDcmDspReadMemoryRangeInfo->DcmDspReadMemorySecurityLevelRefNum != 0u)
    {
        for (index = 0; (index < pDcmDspReadMemoryRangeInfo->
                DcmDspReadMemorySecurityLevelRefNum); index++)
        {
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    if(E_OK == ret)
    {
        ...
        ret = Dcm_0x23ReadMemoryRangeCheck(&Dcm_0x23Type,&MemoryIdInfoIndex,
    ...
    {/*call the callout*/
        ...
        ret = Dcm_ReadMemory(OpStatus,MemoryIdentifier,MemoryAddress,&Memorysize,
                DCM_MemoryData,ErrorCode);
