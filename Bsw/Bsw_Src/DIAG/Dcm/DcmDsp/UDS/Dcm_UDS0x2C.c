        ...
        if (Subfunction == DCM_UDS0X2C_01_DDBYDID)
        {
            ...
            else
            {
                for(Cnt = 0u;(Cnt < RequestNum) && (ret == E_OK);Cnt++)
                {
                    ...
                    if (pDidCfgIndex != 0xFF)
                    {
                        DspInternalUDS0x2C_SourceDidLength(pDidCfgIndex,&Length);
                    }
                    if ((ret == E_NOT_OK) || (Length < (Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[6u + (Cnt * (4u))]
                           + Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[7u + (Cnt * (4u))] - 1u))
