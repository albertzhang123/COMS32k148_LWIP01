                    ...
                    DcmDspRoeEventProperties->DcmDspRoeOnChangeOfDataIdentifier))
    {
        ...
        for (iloop = 0; iloop < DCM_CONNECTION_NUM; iloop++)
        {
            if (Dcm_RoeRuntime.ProtocolCtrlId  ==
                    Dsl_Protocol_ConnectionCfg[iloop].DcmDslParentProtocolRowCtrlId)
            {
                for (index = 0; index < Dsl_Protocol_ConnectionCfg[iloop].
                pDcmDslMainConnection->DcmDslProtocolRx_Num; index++)
                {
                    if (DCM_FUNCTIONAL != Dsl_Protocol_ConnectionCfg[iloop].
                         pDcmDslMainConnection->pDcmDslProtocolRx->DcmDslProtocolRxAddrType)
                    {
                        pduRxId = Dsl_Protocol_ConnectionCfg[iloop].
